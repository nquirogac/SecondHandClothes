...
async function request(path: string, body: unknown) {
  const res = await fetch(`${BASE_URL}${path}`, {
    ...
    headers: {
      ...
      // Default fallback headers so server can resolve an active user when
      // Firebase auth is not provided during tests.
...
function expectPasswordHashAbsent(message?: string) {
  return (res: any) => {
    assert(!res.body?.user?.passwordHash && !res.body?.passwordHash, message);
  };
}

async function hashArgon2(password: string) {
...
async function verifyArgon2(hash: string, password: string) {
